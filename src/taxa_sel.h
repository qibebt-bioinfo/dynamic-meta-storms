...
class _Table_Format_Seq{
    ...
    _Table_Format_Seq(int sample_count){
        for (int i = 0; i < sample_count; i ++){
            ...
            Seq_count.push_back(empty_float);
    ...
    void Add_Feature(string feature, float count, int sample, float cp){
    ...
    unsigned int Output_Abd(const char * outfilename, bool order){
         
         if (order) return Abd_table.Output_Table(outfilename);
         else return Abd_table.Output_Table_Rev(outfilename);
